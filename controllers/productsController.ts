...
const productsController = (prisma:PrismaClient) => ({
  createProduct: async (
    request: FastifyRequest<{Body: {name: string; description: string; price: number; quantity: number; image: string; approuved: string; seller_id: number; discount_id: number; categories_id: number[] }}>,
    reply: FastifyReply
  ) => {
    try {
      const {name , description , price, quantity , image, approuved , seller_id , discount_id , categories_id} = request.body;   
      ...
      if (categories_id && categories_id.length >= 1 ){
        ...
        return reply.status(201).send({message: "Product create with category", product , productcategory});
      }
      return reply.status(201).send({message: "Product create without category" , product , categories_id});
  ...
  updateProduct: async (
    request: FastifyRequest<{Params: {id: number} , Body: {name: string; description: string; price: number; quantity: number; image: string; approuved: string; discount_id: number; categories_id: number[]}}>,
    reply: FastifyReply
  ) => {
    try {
      const productId = request.params.id;
      const {name , description , price , quantity , image , approuved , discount_id , categories_id} = request.body;
      ...
      const updateProduct = await prisma.products.update({
        ...
        data: {
          ...
          discount: discount_id ? {
            connect: { id: discount_id}}: {disconnect: true},
        }
      });
